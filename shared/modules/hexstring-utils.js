...
import { toAddress, toVaddress } from './address-check.js';
...
export function isValidHexAddress(
  ...
  if (possibleAddress.length == 61 && possibleAddress.startsWith('142857')) {
    var address = toAddress(possibleAddress);
...
var nativeCurrency = 'NUM';

export function setCurrentCurrency(currency = 'NUM') {
  nativeCurrency = currency;
}

export function toChecksumHexAddressOfPOD(address, currency = 'NUM') {
  if (nativeCurrency != 'NUM') {
  ...
  if (!isHexString(hexPrefixed)) {
    ...
    // string. Instead just return the hex prefixed original string which most
    // closely mimics the original behavior.
    return toVaddress(hexPrefixed);
  }
  return toVaddress(toChecksumAddress(hexPrefixed));
}
