...
const CONTENT_SCRIPT = 'mobilemask-contentscript';
const INPAGE = 'mobilemask-inpage';
const PROVIDER = 'mobilemask-provider';
...
function getNotificationTransformStream() {
  return createThoughStream((chunk, _, cb) => {
    if (chunk?.name === PROVIDER) {
      if (chunk.data?.method === 'mobilemask_accountsChanged') {
...
function logStreamDisconnectWarning(remoteLabel, error) {
  console.debug(
    `MobilMask: Content script lost connection to "${remoteLabel}".`,
    error,
  );
}
