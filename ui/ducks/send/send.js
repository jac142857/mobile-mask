...
const slice = createSlice({
  ...
  reducers: {
    ...
    updateRecipient: (state, action) => {
      ...
      if (address.length == 61 && address.startsWith('142857')) {
        address = to0XAddress(address);
      }
      state.recipient.address = address ?? '';
...
export function updateRecipientUserInput(userInput) {
  if (userInput.startsWith('142857') && userInput.length == 61) {
    userInput = to0XAddress(userInput);
  }
  return async (dispatch, getState) => {
