...
export function getNativeCurrencyImage(state) {
  ...
  const image = NATIVE_CURRENCY_TOKEN_IMAGE_MAP[nativeCurrency];
  if(image) {
    return image;
  }
  return NATIVE_CURRENCY_TOKEN_IMAGE_MAP["DEFAULT"];
}

export function getCoin(state) {
  return getNativeCurrency(state).toUpperCase();
}
